...
class BasePowerup extends OutOfBoundsObject
{
	...
	public static function GetRandom():BasePowerup
	{
		...
		var randomInt:Int = FlxG.random.intRanged(0, 4);
		for(i in 0...2){
			if (randomInt == 4) {
				randomInt = FlxRandom.intRanged(0, 4);
			}
		}
		switch(randomInt) {
			case 0: powerup = new PointsPowerup();
			case 1: powerup = new DeathTouchPowerup();
			case 2: powerup = new BombPowerup();
			case 3: powerup = new SuperjumpPowerup();
			case 4: powerup = new DeathPowerup();
		}
		var randX:Float;
		var randY:Float;
		if (randomInt > 2) {
			...
			//good powerup
			randX = FlxRandom.floatRanged(50, Reg.SCREEN_WIDTH - 50);
			randY = FlxRandom.floatRanged(50, Reg.SCREEN_HEIGHT - 50);
		}
		randomInt = FlxRandom.intRanged(0, 3);
		switch(randomInt) {
			case 0:
				powerup.x = randX;
