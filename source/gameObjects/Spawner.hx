 ...
 */
 ...
 @:bitmap("assets/images/Spawner.png")
class SpawnerImg extends BitmapData { }
...
class Spawner extends FlxSprite
{
	...
	{
		super(X, Y);
		loadGraphic(SpawnerImg, true, 15, 15);
		animation.add("idle", [0]);
		animation.add("appear", [1, 2, 3, 4, 5, 6, 0], 10,false);
		animation.add("rotate", [8, 9, 10, 0], 10);
		animation.add("mutate", [30, 30, 30, 30, 0, 0, 0, 0, 30, 30, 0, 0, 30, 30, 0, 0, 30, 0, 30, 0, 30, 0, 30, 0],30,false);
		...
		changeEnemy();
		_timer = 0;
		x = FlxG.random.floatRanged(50, Reg.SCREEN_WIDTH - 50);
		y = FlxG.random.floatRanged(50, Reg.SCREEN_HEIGHT- 50);
	}
	...
	private function playSpark():Void
	{
		if(!repositioning && !mutating){
			var name:String = "spark" + FlxG.random.intRanged(1, 5);
			animation.play(name);
		}
	}
	...
	private function Reposition():Void
	{
		if (!repositioning) {
			...
			repositioning = true;
			_timer = 0;
			var newx:Float = FlxG.random.floatRanged(50, Reg.SCREEN_WIDTH - 50);
			var newy:Float = FlxG.random.floatRanged(50, Reg.SCREEN_HEIGHT - 50);
			moveX = (newx - x) / 30;
			moveY = (newy - y) / 30;
		}
	}
	...
	override public function update():Void 
	{
		...
		if (repositioning) {
			...
			if (!mutating && FlxG.random.chanceRoll(0.1)) {
				Reposition();
			}
			if (_timer % 10 == 0)
				playSpark();
		...
		if (mutating) {
			...
			if (!repositioning && FlxG.random.chanceRoll(0.1)) {
				mutating = true;
		...
		if (_timer > SPAWNER_DELAY && FlxG.random.chanceRoll(2)) {
			animation.play("rotate", true);
			_timer = 0;
